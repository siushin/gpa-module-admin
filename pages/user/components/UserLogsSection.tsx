...
import { getUserLogs } from '@/modules/admin/services/user';
import {
  DEFAULT_PAGE_SIZE,
  DEFAULT_PAGINATION,
  TABLE_SIZE,
...
const UserLogsSection: React.FC<UserLogsSectionProps> = ({ accountId }) => {
  ...
  const getConfigByTab = (tab: LogTabKey) => {
    const baseRequest = async (params: any) => {
      ...
      const response = await getUserLogs(requestParams);
